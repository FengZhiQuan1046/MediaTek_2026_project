...
DIN_STEPS="${DIN_STEPS:-30000}"; TRAIN_STEPS="${TRAIN_STEPS:-60000}"
...
run_subset() {
  ...
  args=(--dataset "$dataset" --cache-dir "$CACHE_DIR" --output-dir "$run_dir"
    ...
    --batch-size "$BATCH_SIZE" --eval-batch-size "$EVAL_BATCH_SIZE" --eval-every-steps "$EVAL_EVERY_STEPS"
...
for ((repeat=1; repeat<=REPEATS; repeat++)); do
  jobs=0
  for spec in "Full_Beauty|amazon-all-beauty" "Baby_Products|amazon:Baby_Products" \
              "Sports_and_Outdoors|amazon-sports-and-outdoors" "Toys_and_Games|amazon-toys-and-games"; do
