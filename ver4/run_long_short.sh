...
ENABLE_LORA=1
OUTPUT_GROUP="amazons_long_short"
OUTPUT_ROOT="${OUTPUT_ROOT:-$PROJECT_ROOT/outputs_mamba_rl/$OUTPUT_GROUP}"
SPECIALISTS_EPOCH=3
COORDINATOR_EPOCH=3
JOINT_EPOCH=7
SPECIALISTS_LR=1e-4
...
USE_PREFERENCE="${USE_PREFERENCE:-0}"
USE_GCN="${USE_GCN:-1}"
...
run_subset() {
  ...
  env "${COMMON_ENV[@]}" bash "$PROJECT_ROOT/run_mamba_rl.sh" "$dataset" "$GPU_IDS" \
    ...
    --experiment-note "$OUTPUT_GROUP subset=$subset_name; short filtered-full-sequence <= $EVAL_LENGTH_THRESHOLD; long > $EVAL_LENGTH_THRESHOLD; same model and full catalog" \
    "${TRAINING_OPTIONS[@]}" \
